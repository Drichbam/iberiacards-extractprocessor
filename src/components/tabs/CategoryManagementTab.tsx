...
import { Plus, Download, Upload, Trash2, Edit, ChevronRight, FolderOpen, Folder, Users } from "lucide-react";
...
export default function CategoryManagementTab() {
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      const parsed = parseHierarchicalCSV(content, categories, true);
  ...
  const handleImportConfirm = async () => {
    ...
    try {
      ...
      const deletePromises = [
        ...categories.map(cat => deleteCategory(cat.id, cat.name)),
        ...subcategoriesWithCategories.map(sub => deleteSubcategory(sub.id, sub.name))
      ];
      await Promise.all(deletePromises);

      // Create new categories and subcategories
      for (const cat of parsedImportData.categories) {
        await createCategory({ name: cat.name, color: cat.color });
      }

      // Fetch categories to map names to IDs
      const updatedCategories = await supabase.from('categories').select('*').then(res => res.data as Category[]);

      for (const sub of parsedImportData.subcategories) {
        const category = updatedCategories?.find(c => c.name === sub.category);
        if (category) {
          await createSubcategory({ name: sub.name, color: sub.color, category_id: category.id });
        } else {
          console.warn(`Category "${sub.category}" not found for subcategory "${sub.name}"`);
        }
      }

      toast({
        title: "Import Successful",
        description: `Successfully imported ${parsedImportData.categories.length} categories and ${parsedImportData.subcategories.length} subcategories.`,
        variant: "default",
      });
  ...
  return (
    <div className="space-y-6">
                        ...
                        {subcategories.length > 0 && (
                          <div className="border-t">
                            {subcategories.map((subcategory) => (
                              <div
                                key={subcategory.id}
                                className="flex items-center justify-between p-3 pl-12 hover:bg-muted/30 border-l-2 ml-6"
      ...
      <SubcategoryForm
        ...
        description={editingSubcategory ? "Update subcategory details" : "Create a new subcategory"}
      />
      ...
      <DeleteAllConfirmationDialog
        ...
        onConfirm={async () => {
          ...
          toast({
            ...
            description: "All categories and subcategories have been deleted",
            variant: "success",
          });
        }}
        shopCount={categories.length + subcategoriesWithCategories.length}
      />
      ...
      <ImportConfirmationDialog
        ...
        currentShopCount={categories.length + subcategoriesWithCategories.length}
